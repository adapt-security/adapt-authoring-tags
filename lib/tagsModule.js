...
class TagsModule extends AbstractApiModule {
  ...
  boot(app, resolve, reject) {
    super.boot(app, () => {
      ...
      db.isConnected ? this.init(resolve) : db.on('boot', () => this.init(resolve));
    }, reject);
  }
  init(done) {
    /** @todo this should be in config */
    const targetSchemas = ['course'];
    targetSchemas.length && targetSchemas.forEach(this.extendSchema, this);
    done();
  }
  /**
