...
class TagsModule extends AbstractApiModule {
  ...
  async handleTransfer(req, res, next) {
    const { sourceId, destId } = req.apiData.data
    try {
      await Promise.all(this.modules.map(async m => {
        try {
          await m.updateMany({ tags: sourceId }, { $pull: { tags:sourceId }, $push: { tags: destId } }, { rawUpdate: true })
        } catch (e) {
